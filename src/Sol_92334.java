...
public class Sol_92334 {
    public int[] solution(String[] id_list, String[] report, int k) {
        ...
        for (String r : reportSet) {
            ...
            reporterMap.putIfAbsent(reported, new ArrayList<>());
            reporterMap.get(reported).add(reporter);
        }

        // 4. 정지된 유저 찾고 메일 카운트 증가
        for (String reported : reporterMap.keySet()) {
            List<String> reporters = reporterMap.get(reported);
            if (reporters.size() >= k) {
