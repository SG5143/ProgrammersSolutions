...
public class Sol_42889 {
    public int[] solution(int N, int[] stages) {
        int[] stay = new int[N + 2]; // 스테이지별 머무른 인원
        for (int s : stages) {
            stay[s]++;
        }

        int total = stages.length; // 도달 인원
        ...
        for (int i = 1; i <= N; i++) {
            if (total == 0) {
                ...
                failRate[i] = (double) stay[i] / total;
                total -= stay[i];
            }
        }
        ...
        Arrays.sort(stagesArr, (a, b) -> {
            if (failRate[b] == failRate[a]) {
                return a - b; // 실패율 같으면 번호 오름차순
            }
            return Double.compare(failRate[b], failRate[a]); // 실패율 내림차순
        });
    ...
    public static void main(String[] args) {
        ...
        int[] result = new Sol_42889().solution(n, stages);
